// Synchronous Module Definition 同步模块模式
// 排队开发
...
var F = F || {};
/**
 * 定义模块方法(理论上，模块方法应放在闭包中实现，可以隐藏内部消息，这里我们为让读者能够看明白，我们忽略此步骤)
 * @param str 模块路由
 * @param fn 模块方法
 */
...
// test
// 注意，在真正的模块开发中，是不允许这样直接调用的，有两点原因，技术上，模块通常保存在闭包内部
// 的私有变量李，而不会保存在F上，因此是获取不到的，而我们这里简化了闭包，也为测试方便，
// 因此直接引用。其次，类似如下调用是不符合模块化开发规范的
F.string.trim('测试用例。 ')
// 
...
F.dom('test').html();
// 对于模块的创建，我们也可以先声明后创建，如添加addClass()为元素添加class方法
// 为dom模块添加addClass 方法
// 注意，此时添加模式之所以可行，是因为将模块添加到F对象上，模块开发中只允许上面的添加方式
